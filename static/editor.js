...
quick.addEventListener('click', e => {
  ...
  if (b) setColour(b.dataset.colour);
});
...
function contrast(hex) {
  const r = hexToRgb(hex) || [0,0,0];
  return (r[0]*299+r[1]*587+r[2]*114)/1000 > 150 ? '#111' : '#fff';
}
...
function setColour(hex) {
  ...
  document.getElementById('rInput').value = rgb[0];
  document.getElementById('gInput').value = rgb[1];
  document.getElementById('bInput').value = rgb[2];
}
...
document.getElementById('hexInput').addEventListener('change', e => {
  ...
  if (r) setColour(rgbToHex(r));
  else toast(t('invalid_hex'), true);
});

for (const id of ['rInput','gInput','bInput']) {
  document.getElementById(id).addEventListener('change', () => setColour(rgbToHex([
...
function updateSelection() {
  ...
  document.getElementById('selectionNames').textContent =
    selected.size ? [...selected].join(', ') : t('click_keys');

  document.querySelectorAll('.key[data-key]').forEach(k => {
    k.classList.toggle('selected', selected.has(k.dataset.key));
  });
}
...
function selectOnly(ids) {
  selected.clear();
  for (const id of ids) {
    if (state?.keys[id]?.editable) selected.add(id);
  }
  updateSelection();
}
  ...
  for (const row of rows) {
    ...
    for (const item of row) {
      if (item.gap) {
        ...
        gap.style.setProperty('--u', item.gap);
      ...
      key.className = 'key';
      key.dataset.key = item.id;
      key.style.setProperty('--u', item.w || 1);
      ...
      if (info.dynamic) {
        key.classList.add('dynamic');
        key.disabled = true;
        key.title = t('dynamic_title');
      } else if (!info.mapped) {
        key.classList.add('unknown');
        key.disabled = true;
        key.title = t('no_mapping_title');
      } else {
        key.style.background = info.hex;
        key.style.color = contrast(info.hex);
        key.title = `${item.label} · ${info.hex}`;

        key.addEventListener('click', () => {
          if (selected.has(item.id)) selected.delete(item.id);
          else selected.add(item.id);

          if (selected.size === 1) setColour(info.hex);
          updateSelection();
        });
      }
...
function renderPalette() {
  ...
  document.getElementById('paletteCount').textContent =
    t('colours_count', {count: state.palette.length});

  root.innerHTML = state.palette.map((p,i) =>
    `<button class="palette-swatch" data-index="${i}" title="${p.hex}" style="background:${p.hex}"><span>${p.count}</span></button>`
  ).join('');

  root.onclick = e => {
    const sw = e.target.closest('[data-index]');
    if (!sw) return;

    const p = state.palette[Number(sw.dataset.index)];
    const ids = Object.entries(state.keys)
      .filter(([,v]) => v.editable && v.hex === p.hex)
      .map(([k]) => k);

    selectOnly(ids);
    setColour(p.hex);

    root.querySelectorAll('.palette-swatch').forEach(x => {
...
async function loadState() {
  state = await api(`/api/profiles/${profileId}/keyboard`);
  renderKeyboard();
  renderPalette();
...
document.querySelectorAll('[data-group]').forEach(b => {
  b.addEventListener('click', () => selectOnly(state.groups[b.dataset.group] || []));
});

document.getElementById('allMappedBtn').addEventListener('click', () => {
  selectOnly(Object.entries(state.keys).filter(([,v]) => v.editable).map(([k]) => k));
});
...
document.getElementById('applyBtn').addEventListener('click', async () => {
  if (!selected.size) {
    toast(t('select_keys_first'), true);
    return;
  }

  const rgb = hexToRgb(document.getElementById('hexInput').value);
  if (!rgb) {
    toast(t('invalid_colour'), true);
    return;
  }

  try {
    const data = await api(`/api/profiles/${profileId}/keys`, {
      method:'POST',
      body:JSON.stringify({keys:[...selected], rgb})
    });
    ...
    toast(data.changed.length
      ? t('changed', {keys:data.changed.join(', ')})
      : t('colour_already_set'));
  } catch (e) {
...
async function history(direction) {
  try {
    await api(`/api/profiles/${profileId}/${direction}`, {method:'POST'});
    selected.clear();
    await loadState();
    toast(direction === 'undo' ? t('undone') : t('redone'));
  } catch (e) {
    toast(e.message, true);
  }
}
